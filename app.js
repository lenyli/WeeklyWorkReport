...
const HELP_CONTENT = {
  member: [
    ...
    ["提交周报", "把当前周报提交到本机数据库。再次提交会覆盖本人本周的旧版本。"],
    ["修改密码", "在页面右上角修改密码。密码必须正好是六位数字。"],
    ["下载个人 Excel", "把当前填写内容下载成个人周报 Excel。"],
  ],
  leader: [
    ["汇总周一日期", "打开后会自动切到本周周一，可以手动切换要汇总的周。"],
    ["上传成员 Excel", "选择成员发来的个人周报 Excel，页面会在本地解析并加入汇总。"],
    ["成员账号", "成员第一次使用姓名和初始密码 123456 登录时自动创建账号；删除账号不会删除历史周报。"],
    ["提交情况", "按照当前账号和保留的历史周报显示已提交和未提交人员。"],
...
async function login(name, password) {
  ...
  if (!/^\d{6}$/.test(password)) throw new Error("密码必须是六位数字。");
  const account = await LocalDatabase.authenticateOrCreate(normalized, password);
  if (!account) throw new Error("姓名或密码错误。");
  currentUser = {
    name: normalized,
    role: account.role,
...
function openPasswordDialog() {
  ...
  $("passwordStatus").textContent = "密码必须是六位数字。";
...
async function changeCurrentPassword() {
  ...
  if (!/^\d{6}$/.test(oldPassword) || !/^\d{6}$/.test(newPassword)) throw new Error("密码必须是六位数字。");
  if (newPassword !== confirmation) throw new Error("两次输入的新密码不一致。");
  if (oldPassword === newPassword) throw new Error("新密码不能与原密码相同。");
  await LocalDatabase.changePassword(currentUser.name, oldPassword, newPassword);
  $("passwordDialog").close();
  $("reportStatus").textContent = "密码已修改。";
}
...
async function submitReport() {
  ...
  await LocalDatabase.saveSubmission(submission);
  clientState.submissions[submissionKey(submission.weekStart, submission.memberName)] = submission;
  saveClientState();
  $("reportStatus").textContent = `已提交，提交时间：${formatDateTime(submission.submittedAt)}。再次提交会覆盖本周版本。`;
  if (currentUser.role === "collector") await refreshCollector();
}
...
async function refreshCollector() {
  ...
  const [users, storedReports] = await Promise.all([
    LocalDatabase.listUsers(),
    LocalDatabase.listSubmissions(weekStart),
  ]);
  const localData = getLocalCollectorData(weekStart);
  storedReports.forEach((report) => {
    localData.reports[submissionKey(report.weekStart, report.memberName)] = report;
  });
  const reports = localData.reports;
  collectorData = {
    users: users.filter((user) => user.role === "member"),
    roster: sortNames([
      ...users.filter((user) => user.role === "member" && user.active).map((user) => user.name),
      ...Object.values(reports).map((report) => report.memberName),
    ]),
    reports,
  };
  renderCollector();
...
function renderCollector() {
  ...
  const users = collectorData.users || [];
  $("rosterCount").textContent = `(${users.length} 个账号)`;
  $("rosterList").replaceChildren(
    ...sortNames(users.map((user) => user.name)).map((name) => {
      const user = users.find((item) => item.name === name);
      const row = document.createElement("div");
      row.className = "person-row";
      row.innerHTML = `<span><strong></strong></span><button class="ghost danger" type="button">删除</button>`;
...
async function deleteMember(name) {
  if (!confirm(`确定删除 ${name} 的账号？已提交的历史周报会保留。`)) return;
  await LocalDatabase.deleteUser(name);
...
function bindEvents() {
  ...
  $("clearBtn").addEventListener("click", clearCurrentReport);
